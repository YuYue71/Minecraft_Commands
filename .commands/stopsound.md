# `/stopsound`（停止音效指令）
* > 用於停止正在對特定玩家播放的遊戲內音效
* > 可精確指定要切斷的聲道分類，或是直接指定特定的聲音識別碼（Sound ID）
* > 常用於地圖製作、劇情切換、背景音樂（BGM）中斷與過場動畫的音效控制

---

## 語法結構 (Syntax)
```commands id="stopsound"
/stopsound <目標> <聲道> <聲音名稱>
```
...
| --- | --- |
| `[必填]` `<目標>` | 指定要停止音效的目標玩家 |
| `[選填]` `<聲道>` | 指定要停止哪一個音量分類管道的聲音（若省略則停止該玩家聽到的所有聲音） |
| `[選填]` `<聲音名稱>` | 指定要停止的特定聲音識別碼（若指定，則只有該聲音會被切斷） |
